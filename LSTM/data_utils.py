...
class CropDataset:
    ...
    def get_next_batch(self, use_data='train', batch_size=100):
        ...
        positives = data[data.label_id == 1].iloc[:, :-1] 
        negatives = data[data.label_id == 0].iloc[:, :-1]

        
        positives = positives.sample(frac=1)

        pos_batch_size = batch_size // 2  #floor division 
        ...
        while start < num_positive_samples:
            ...
            start += batch_size
            print("batchxtranspose:", batchX.transpose(2, 1))
